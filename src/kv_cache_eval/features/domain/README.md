...
입력 크기의 기본 한도는 평가·검토 요청 각각 **131,072 UTF-8 바이트(128 KiB)**입니다. 실제 재조사 결과에서 기술별 원문·메모가 30 KiB를 넘고 응답 스키마·근거 검토 여유분까지 더해져 이전 32,000바이트 한도에서는 두 기술 모두 LLM 호출을 생략했습니다. 같은 State를 `max_input_bytes=131072`로 재평가했을 때 도메인 평가가 진행된 범위를 기본값으로 적용했습니다. 두 기술의 근거가 한 요청에 들어가지 않으면 기술별로 나누고, 한 기술의 근거도 한도를 넘으면 해당 기술의 평가를 미확인으로 반환합니다. 호출별 제한이 필요하면 `evaluate(state, max_input_bytes=...)`를 직접 지정할 수 있습니다. 이 평가는 공개 근거에 대한 판단이며 실제 GPU 운영 측정 결과를 뜻하지 않습니다.
