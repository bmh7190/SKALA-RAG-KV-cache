"""KIVI·InfiniGen PDF 색인과 공통 RAG/Tavily 조사를 실행한다."""
...
from kv_cache_eval.features.technical_research.node import make_runtime_llm
...
def main() -> None:
    ...
    common.add_argument("--technology", choices=("KIVI", "InfiniGen"), default="InfiniGen")
    ...
    evaluation.add_argument("--output-name", default="retrieval_eval_shared.json")
    ...
    research.add_argument("--max-llm-calls", type=int, default=24)
    research.add_argument("--questions", type=int, default=10, help="앞에서부터 조사할 질문 수, 1~10")
    research.add_argument("--question-id", help="한 질문 ID만 조사; --questions보다 우선")
    research.add_argument("--output-name", default="research_result_web.json", help="data/cache/<technology>/ 안의 결과 파일명")
    args = parser.parse_args()
    if args.command == "evaluate" and args.technology != "InfiniGen":
        parser.error("고정 검색 평가 질문집은 InfiniGen 전용입니다")
    if args.command != "index" and (Path(args.output_name).name != args.output_name
                                    or not args.output_name.endswith(".json")):
        parser.error("--output-name은 경로 없는 .json 파일명이어야 합니다")
    load_environment()
    cache_dir = DEFAULT_CACHE / args.technology.lower()
    output_file = cache_dir / args.output_name if args.command != "index" else None
    if output_file is not None and output_file.exists():
        parser.error(f"기존 결과를 보존하기 위해 덮어쓰지 않습니다: {output_file}")
    ...
    if args.command in ("index", "evaluate"):
        retriever, index_info = ensure_index(args.technology, settings=settings)
    ...
    if args.command == "evaluate":
        ...
        return
    all_questions = questions_for_state(new_state(), target=args.technology)
    ...
    def web_search(query: str, count: int):
        return search_web(query, args.technology, count)

    reviewer = ModelReviewer(llm, args.max_llm_calls, target=args.technology)
    result = research_questions(questions, rag_search, reviewer, target=args.technology,
